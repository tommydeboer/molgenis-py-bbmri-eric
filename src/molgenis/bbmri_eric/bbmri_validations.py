...
def validate_bbmri_id(entity, nn, bbmriId):
    ...
    if not re.search(idRegex, bbmriId) and not re.search(
        ...
        errors.append(
            f"""{bbmriId} in entity: {entity} does
...
def _validate_id_in_nn_entry(
    ...
    if not validate_bbmri_id(entity=entity, nn=nn, bbmriId=bbmriId):
        print(
            f"""{parent_entry['id']} in entity: {parent_entity}
            contains references to entity: {entity} with an invalid id ({bbmriId})"""
        )
        return False
    else:
        return True
...
def validate_refs_in_entry(
    nn: dict, entry: dict, parent_entity: str, possible_entity_references: list
):
    ...
    for entity_reference in possible_entity_references:
        ...
        if type(refData) is dict:
            return _validate_id_in_nn_entry(
            ...
            for ref in refData:
                if type(ref) is dict:
                    ...
                    if not validate_bbmri_id(
                        entity=entity_reference, nn=nn, bbmriId=ref
                    ):
                        return False
    return True
